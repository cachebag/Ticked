        ...
        with Container(classes="calendar-setup-container"):
            with Vertical(classes="calendar-setup-form"):
                ...
                with Vertical():
                    ...
                    yield Input(value=config['url'] if config else "", placeholder="https://caldav.example.com", id="server-url")
                
                with Vertical():
                    yield Label("Username")
                    yield Input(value=config['username'] if config else "", id="username")
                
                with Vertical():
                    yield Label("Password")
                    yield Input(value=config['password'] if config else "", password=True, id="password")
                    ...
                    if config:
                        ...
                        if sync.connect(config['url'], config['username'], config['password']):
                            ...
                            if calendars:
                                ...
                                yield Select(options=calendar_options, value=config['selected_calendar'], id="calendar-select")
                            else:
