...
class FilterableDirectoryTree(DirectoryTree):
    ...
    def on_mouse_down(self, event: MouseDown) -> None:
        if event.button == 3: 
            try:
                offset = event.offset
                node = self.get_node_at_line(offset.y - 1) # offset might come with padding so we -1 to get the actual line
        ...
        if tree:
            for row in range(tree.row_count):
                ...
                if node and node.data.path == self.path:
                    ...
                    menu_x = tree_offset.x + min(tree.scrollable_content_region.width - 20, 
